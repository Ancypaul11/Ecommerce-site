...
import { CartData } from '../../../shared/data/cart-data';
import { CoustomerData } from '../../../shared/data/customer-data';
...
export class CustomerOrdersComponent implements OnInit {
  ...
  constructor(
    ...
    if (localData !== null) {
      ...
      this.getCartByCustomerId(this.loggedInObj[0].custId);
    }
    this.logSrv.cartUpdated$.subscribe((res: any) => {
      if (res) {
        this.getCartByCustomerId(this.loggedInObj[0].custId);
      }
    });
  ...
  getSaleByCustId() {
    ...
    this.saleList = [];
    CoustomerData.forEach((data) => {
      if(data.custId == this.loggedInObj[0].custId) {
